...
class CCTCSpider(BaseSpider):

    def __init__(self, model, spider_params=None) -> NoReturn:
    ...
    def parse_item(self, position_data):
        data = {
            # 代表岗位名称
            'job_title': position_data['name'],
            # 类型 0 校招 1 实习 2 社招
            'class': self.cls,
            # 工资
            'salary': position_data.get('salary'),
            # 职责
            'responsibility': position_data['duty'],
            # 要求
            'raw_position_require': position_data['requirement'],
            # 解析以后的要求
            'position_require': {
                # 有的可以解析出来学历
                'degree': None,
            ...
            # 工作类型
            'job_type': self.post_type.get(position_data.get('type')),
            # 岗位 id
            'post_id': position_data['id'],
            # 公司名称或者是部门名称或者是项目名称
            'company_name': position_data.get('projectName'),
            # 该条数据对应的网页 url，别人点开这个链接，能打开这个岗位
            'position_web_url': self.position_web_url,
            # 招聘人数
            'num_hire': position_data.get('leftCount'),
            # 不用动
    ...
    async def _parser(self, request_params: Dict[str, Any], response: Dict[str, Any]):
        ...
        try:
            if response.get('code') == 200 and response.get('data'):
                data = self.parse_item(response['data'])
            else:
    ...
    async def _parser_page_list(self, request_params, response):
        ...
        try:
            ...
            if response.get('code') == 200:
                for position_data in (response.get('data', {}).get('showList', []) or []):
                    flag = True
                    ...
                    self.position_web_url = self.position_web_url_format.format(self.post_id)
    ...
    async def _downloader_page_list(self, request_params):
        ...
        self.position_list_req_data['pageNum'] = self.page_index + 1
        res = await requests.post(self.position_url,
    ...
    async def run_page_list(self, page_config):
        ...
        for position_info in [({
                               ...
                               }, 0, 'https://hr.cctc.cc/post?id={}&projectType=1'),
                              ({
                                   'projectId': [],
                                   'adminProjectType': 2,
                                   'pageNum': 1,
                                   'pageSize': self.page_size
                              }, 2, 'https://hr.cctc.cc/post?id={}&projectType=3'),
                              ({
                                   'projectId': [37],
                                   'pageNum': 1,
                                   'pageSize': self.page_size
                               }, 3, 'https://hr.cctc.cc/post?id={}&projectType=1')]:

            self.position_url = page_config['page_list_url']
